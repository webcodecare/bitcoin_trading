...
export default function AdvancedForecastChart({ ticker }: AdvancedForecastChartProps) {
  ...
  const { data: modelMetrics = [], isLoading: modelsLoading } = useQuery({
    queryKey: [`/api/forecast/models/${ticker}`],
  });
  ...
  const generateForecast = useMutation({
    mutationFn: async (horizon: number) => {
      ...
      const response = await apiRequest("POST", `/api/forecast/advanced/${ticker}`, {
        horizon
      });
